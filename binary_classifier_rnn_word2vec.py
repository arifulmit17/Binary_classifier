...
########################################### Word2vec on the dataset ####################################################
        def word2vec_embed(text_1,word_index,NUM_WORDS,embedding_dim):
              ...
              token=[]
              for i in text_1.index:
                   token.append(nltk.word_tokenize(text_1[i]))
             
              word_vectors = Word2Vec(token,size=EMBEDDING_DIM, window=5, min_count=1,sg=1)
        ...
        model = Sequential()
        model.add(Embedding(vocabulary_size,EMBEDDING_DIM, input_length=sequence_length,mask_zero=True,weights=[embedding_matrix] ,trainable=False))
